...
!-----------------------------------------------------------------------
      include './Subs.f'
!
      subroutine Hypo(stressNew,stateNew,defgradNew,nmax,
     +               stressOld,stateOld,defgradOld,dt,props,
     +               nblock,ndir,nshr,nstatev,nprops,Dissipation)
!-----------------------------------------------------------------------
      implicit none
!-----------------------------------------------------------------------
      integer nblock, ndir, nshr, nstatev, nprops,nmax
      real*8 dt
      real*8 props(nprops),defgradOld(nmax,9),
     +  stressOld(nmax,6),
     +  stateOld(nmax,nstatev),
     +  defgradNew(nmax,9),
     +  stressNew(nmax,6), stateNew(nmax,nstatev)
!-----------------------------------------------------------------------
!     	Internal vumat variables
!-----------------------------------------------------------------------
!     Material & other parameters
      ...
      real*8 Dissipation(nmax)! The change in dissipated inelastic specific energy (sigma_ij*D^p_ij*dt=sum(tau(alpha)*dgamma(alpha)))
      real*8 ang(3)! Euler angles phi1, PHI, phi2
      real*8 four, three, two, one, half, zero
      real*8 Pi
      parameter (Pi=4.d0*atan(1.d0))
      parameter(four=4.d0, three=3.d0, two=2.d0, one=1.d0,
     +          half=5d-1, zero=0.d0)! Constants
...
!-----------------------------------------------------------------------
      ...
      phi1       = props(9)*Pi/180.d0! Euler angle phi1 in radians
      PHI        = props(10)*Pi/180.d0! Euler angle PHI in radians
      phi2       = props(11)*Pi/180.d0! Euler angle phi2 in radians
...
!-----------------------------------------------------------------------
      ...
      n(8,1:3) = (/-one, one, one/)
      n(9,1:3) = (/-one, one, one/)
      n(10,1:3)= (/ one,-one, one/)
      n(11,1:3)= (/ one,-one, one/)
      n(12,1:3)= (/ one,-one, one/)
!
      m        = zero
      m(1,1:3) = (/-one, zero, one/)
      m(2,1:3) = (/-one, one, zero/)
      m(3,1:3) = (/ zero,-one, one/)
      m(4,1:3) = (/ zero, one, one/)
      m(5,1:3) = (/-one, one, zero/)
      m(6,1:3) = (/ one, zero, one/)
      m(7,1:3) = (/ one, zero, one/)
      m(8,1:3) = (/ one, one, zero/)
      m(9,1:3) = (/ zero,-one, one/)
      m(10,1:3)= (/ zero, one, one/)
      m(11,1:3)= (/ one, one, zero/)
      m(12,1:3)= (/-one, zero, one/)
!
      n = n/sqrt(three)
      m = m/sqrt(two)
!
