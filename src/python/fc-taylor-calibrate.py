...
def yieldfunction3D(s11, s22, s33, s12, s23, s31, c, exponent):
    ...
    s1 = np.linalg.eigvalsh(
        np.array([[x1, xy1, xz1], [xy1, y1, yz1], [xz1, yz1, z1]]))
    s2 = np.linalg.eigvalsh(
        np.array([[x2, xy2, xz2], [xy2, y2, yz2], [xz2, yz2, z2]]))
...
def Normalize(s11, s22, s33, s12, s23, s31):
    ...
    s0 = np.sqrt(0.5*(s11[k]-s22[k])**2+0.5*(s22[k]-s33[k])**2+0.5 *
                 (s33[k]-s11[k])**2+3*(s12[k])**2+3*(s23[k])**2+3*(s31[k])**2)
...
def OptimizeBasinhopping(s11, s22, s33, s12, s23, s31, choice, exponent):
    ...
    for i in range(0, 5):
        res = optimize.basinhopping(ObjFun, c0, niter=niter, T=T, stepsize=stepsize+0.1*i, minimizer_kwargs={
                                    'method': solver, 'args': (s11, s22, s33, s12, s23, s31, YieldFunc, exponent)}, disp=disp)

        ctrial = res.x
        ftrial = ObjFun(ctrial, s11, s22, s33, s12,
                        s23, s31, YieldFunc, exponent)
...
def OptimizeMinimize(s11, s22, s33, s12, s23, s31, choice, exponent):
    ...
    res = optimize.minimize(ObjFun, c0, args=(s11, s22, s33, s12, s23, s31, YieldFunc,
                                              exponent), method=solver, options={'maxiter': niter, 'disp': disp})
...
def OptimizeLS(s11, s22, s33, s12, s23, s31, choice, exponent):
    ...
    res = optimize.least_squares(ObjFunVec, c0, args=(
        s11, s22, s33, s12, s23, s31, YieldFunc, exponent), method=solver)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Calibrate the Yld2004-18p yield surface from discrete yield surface data.')
